...
static void toggletag(uint32_t);
static void toggleview(uint32_t);
...
void chvt(uint32_t ui) { wlr_session_change_vt(session, ui); }
...
void toggletag(uint32_t ui) {
  uint32_t newtags;
  Client *sel = focustop(selmon);
  if (!sel || !(newtags = sel->tags ^ (ui & TAGMASK)))
    return;
